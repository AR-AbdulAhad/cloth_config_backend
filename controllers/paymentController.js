    ...
    export const createCheckoutSession = async (req, res) => {
        try {
            ...
            console.log(`📥 Raw request body:`, JSON.stringify({ orderData: orderData ? 'present' : 'missing', amount }, null, 2));
            ...
            console.log(`💰 Payment processing:
                Raw amount from frontend: ${amount} (${typeof amount})
                Parsed final total: ${finalTotal} DKK
            ...
            console.log(`🔍 Order check: ${existingOrder ? 'Existing order found' : 'New order'}`);
            if (existingOrder) {
                console.log(`   Existing order total: ${existingOrder.total_amount} DKK`);
                console.log(`   Existing order paid: ${existingOrder.amount_paid || 0} DKK`);
            }
            ...
            console.log(`🎯 Final calculation:
                Balance due: ${balanceDue} DKK
                Stripe unit_amount: ${Math.round(balanceDue * 100)} öre
                This should show as: DKK ${(Math.round(balanceDue * 100) / 100).toFixed(2)} in Stripe`);
            ...
            console.log(`🚀 Creating Stripe session:
                Line item unit_amount: ${stripeUnitAmount} öre
                Expected display: DKK ${(stripeUnitAmount / 100).toFixed(2)}`);
        ...
        if (event.type === "checkout.session.completed") {
            ...
            try {
                ...
                if (order) {
                    ...
                    console.log(`✅ Order ${order_id} updated: Paid ${amountPaidThisSession} DKK. Edit deadline set to ${editDeadline}`);
...
export const testAmount = async (req, res) => {
    try {
        ...
        console.log(`🧪 Test amount endpoint:
            Raw amount: ${amount} (${typeof amount})
            Parsed: ${parseFloat(amount)}
            Math.round * 100: ${Math.round(parseFloat(amount) * 100)}
            Back to DKK: ${(Math.round(parseFloat(amount) * 100) / 100).toFixed(2)}`);
